...
public:
	...
	const GameConfig::Position& getPosition() const { return pos; }		// Get the current position of the point
	...
	const GameConfig::Direction& getDir() const { return dir; }			// Get the current movement direction
	...
	const GameConfig::Direction& getPreviousDir() const { return previous_dir; }                   // Get the previous movement direction
	const GameConfig::Direction& getDirBeforeFalling() const { return dir_before_falling; }        // Get the direction before falling
	...
	const GameConfig::Direction& getDirFromDirectionsArray(int i) { return directions[i]; }        // Get a direction from the directions array
	const GameConfig::Direction& getDirBeforeStay() { return dir_before_stay; }                    // Get the direction before staying
